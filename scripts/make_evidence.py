...
Regenerate everything under evidence/ from scratch.
    ...
    python scripts/make_evidence.py            real discovery run (needs ANTHROPIC_API_KEY)
    python scripts/make_evidence.py --scripted stand-in model, for checking the pipeline
...
def main() -> int:
    ...
    ap.add_argument("--scripted", action="store_true", help="use the scripted stand-in model instead of the LLM")
    args = ap.parse_args()
    base_url = os.environ.get("MERIDIAN_URL", "http://127.0.0.1:5057")
    if not args.scripted and not os.environ.get("ANTHROPIC_API_KEY"):
        # the CLI loads .env itself; check the same way it would
        env = (ROOT / ".env").read_text(encoding="utf-8") if (ROOT / ".env").exists() else ""
        if "ANTHROPIC_API_KEY=" not in env or "ANTHROPIC_API_KEY=\n" in env:
            print("ANTHROPIC_API_KEY is not set; pass --scripted to use the stand-in model")
            return 2
    reset_app(base_url)
    shutil.rmtree(RUNS, ignore_errors=True)
    EVIDENCE.mkdir(exist_ok=True)
    index: list[str] = []
    model_args = ["--scripted", "scripts/scripted_model_lookup.json"] if args.scripted else []
    ...
    model_args = ["--scripted", "scripts/scripted_model_subaccount.json"] if args.scripted else []
    ...
    model_note = "the scripted stand-in model (pipeline check only)" if args.scripted else "Claude through the Anthropic API"
    readme = ["# Evidence", "",
